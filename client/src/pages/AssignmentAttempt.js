...
const AssignmentAttempt = () => {
    ...
    useEffect(() => {
        const fetchAssignment = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/assignments/${slug}`);
    ...
    const fetchHistory = async () => {
        try {
            ...
            const res = await axios.get(`http://localhost:5000/api/attempts?assignmentId=${assignment._id}`, config);
            setHistory(res.data);
        } catch (err) {
